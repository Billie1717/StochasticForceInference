...
class ForceGNN(tf.keras.Model):
    ...
    def __init__(self, hidden_dim=32, tau_init=1.0):
        ...
        # Learnable memory timescale (initialized to tau_init)
        self.tau_raw = tf.Variable(
            initial_value=tf.constant(tau_init, dtype=tf.float32),
            trainable=True,
            name='tau_raw'
        )
    ...
    def call(self, inputs, return_components=False):
        ...
        with tf.autodiff.ForwardAccumulator(
            ...
            F_env_v = self.env_net(positions)   # (N,2)
        
        Jv = acc.jvp(F_env_v)   
            # (N,2)  == (∂F/∂x) v
        vel_term = self.tau_raw * Jv - (1.0 / self.tau_raw) * velocities

        F_drag = vel_term * (2/3) / y_std
    ...
    if naive_form:
        ...
        for pid in particles:
            ...
            for i in range(1, len(indices) - 1):
                ...
                data.loc[idx, 'vx'] = (data.loc[idx, 'x'] - data.loc[idx_prev, 'x']) / (2*dt)
                data.loc[idx, 'vy'] = (data.loc[idx, 'y'] - data.loc[idx_prev, 'y']) / (2*dt)
...
def create_model(hidden_dim=32,tau_init=10.0):
    """
    ...
    print(f"ForceGNN model created with hidden_dim={hidden_dim}, tau_init={tau_init}")
    return model
...
def train_model(
    ...
    learning_rate=1e-3,
    verbose=True,
    eval_before_update=True,          # <-- key: same weights for train/val metrics
    ...
    history = {"train_loss": [], "val_loss": [], "train_mse": [], "val_mse": [], "train_reg": [], "val_reg": [], "tau_raw": []}
    ...
    for epoch in range(num_epochs):
        ...
        if eval_before_update:
            trL, trM, trR, trN, trF = compute_loss_on_dataset(train_data, r_far_phys, training_flag=False, include_far=True)
            vaL, vaM, vaR, vaN, vaF = compute_loss_on_dataset(val_data,   r_far_phys, training_flag=False, include_far=include_far_field_in_val)
        else:
        ...
        # ---- Evaluate AFTER updates (standard behavior) ----
        ...
        if hasattr(model, 'tau_raw'):
            history["tau_raw"].append(model.tau_raw.numpy())
        else:
            history["tau_raw"].append(np.nan)
        ...
        if verbose and (epoch % 10 == 0 or epoch == num_epochs - 1):
            ...
            print(
                ...
                f"Tau_raw={model.tau_raw.numpy():.6f}"
            )
...
def get_tau(model):
    ...
    --------
    float
        The learned memory time tau
    """
    tau_raw = float(model.tau_raw.numpy())
    tau = tau_raw  #+ 1e-4
    return tau
...
def load_model(filepath, hidden_dim=None, gamma_init=None, save_format='weights'):
    ...
    if save_format in ['weights', 'h5']:
        ...
        if tau_init is None:
            ...
            tau_init = 10.0
            print("Warning: tau_init not provided, using 10.0. The loaded weights will override this.")
        
        # Recreate model structure
        model = create_model(hidden_dim=hidden_dim, tau_init=tau_init)
        ...
        print(f"  hidden_dim={hidden_dim}, tau_init={tau_init}")
        print(f"  Learned tau value: {get_tau(model):.6f}")
        return model
    elif save_format in ['savedmodel', 'tf']:
