    ...
    if naive_form:
        ...
        for pid in particles:
            ...
            for i in range(1, len(indices) - 1):
                ...
                if 'x_unwrapped' in data.columns:
                    data.loc[idx, 'vx'] = (data.loc[idx, 'x_unwrapped'] - data.loc[idx_prev, 'x_unwrapped']) / (dt)
                    data.loc[idx, 'vy'] = (data.loc[idx, 'y_unwrapped'] - data.loc[idx_prev, 'y_unwrapped']) / (dt)
                    
                    # Acceleration
                    data.loc[idx, 'ax'] = (data.loc[idx_next, 'x_unwrapped'] - 2*data.loc[idx, 'x_unwrapped'] + data.loc[idx_prev, 'x_unwrapped']) / (dt**2)
                    data.loc[idx, 'ay'] = (data.loc[idx_next, 'y_unwrapped'] - 2*data.loc[idx, 'y_unwrapped'] + data.loc[idx_prev, 'y_unwrapped']) / (dt**2)
                else:
                    data.loc[idx, 'vx'] = (data.loc[idx, 'x'] - data.loc[idx_prev, 'x']) / (dt)
                    data.loc[idx, 'vy'] = (data.loc[idx, 'y'] - data.loc[idx_prev, 'y']) / (dt)
