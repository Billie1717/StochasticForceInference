...
def compute_weak_derivatives(
    ...
    w_sym : sympy expression
        ...
        w(±1)=0 and w'(±1)=0 to eliminate boundary terms.
    shift : int, default 0
        Number of frames to shift the *velocity* window backward relative
        to the acceleration window (acceleration always stays centered at
        the reported frame). shift=0 reproduces the original symmetric
        estimator, which is exactly degenerate for gamma_eff inference:
        centering v and a at the same point with the same even test
        function gives <v.a>=0 in expectation for any stationary process
        (both estimators have definite, opposite time-parity about the
        same center). Shifting the v-window backward by `shift` frames
        breaks that degeneracy while keeping the same smoothing kernel
        (and hence the same measurement-noise robustness) on each side.
        See Simulations/Run_WeakDerivative_Pipeline.ipynb (Sections 6-8) for
        the derivation and validation, and `find_matching_shift_frames`
        below for how to choose `shift` given a window size `tau` and an
        (estimated) physical persistence time.
    ...
    if shift:
        if shift < 0:
            raise ValueError("shift must be >= 0 (it shifts the velocity window backward).")
        # Relabel the (still-symmetric) velocity window from frame c to
        # frame c+shift, so the row for frame c now holds the velocity
        # estimate from a window centered `shift` frames earlier, while
        # ax_col/ay_col are left untouched (still centered at c). This is a
        # pure re-indexing -- no new numerical integration -- equivalent to
        # sliding the same smoothing kernel to an earlier reference point,
        # the smoothed analogue of a backward finite difference.
        data = data.sort_values([particle_col, frame_col])
        for col in (vx_col, vy_col):
            data[col] = data.groupby(particle_col, sort=False)[col].shift(shift)
        # Rows that shifted in from before the start of a particle's track
        # (no earlier window available) become NaN from .shift(); treat
        # them the same as the existing zero-filled boundary convention.
        data[[vx_col, vy_col]] = data[[vx_col, vy_col]].fillna(0.0)

    # Print statistics
    total_boundary = total_points - total_computed
    boundary_fraction = total_boundary / total_points if total_points > 0 else 0.0
    print(f"  Weak derivative statistics (tau={tau}, shift={shift}):")
    print(f"    Total points: {total_points}")
    print(f"    Computed (valid centers): {total_computed}")
    print(f"    Boundary points (zeros): {total_boundary}")
    print(f"    Fraction boundary: {boundary_fraction:.4f}")
...
def compute_backward_weak_velocity(
    data: pd.DataFrame,
    dt: float,
    tau: int,
    w_sym,
    ...
    """
    Weak-form velocity using a backward (left-shifted) window.

    Instead of centring the integration window on the evaluation point, the
    window spans [n-tau, n] so the evaluation point sits at the RIGHT edge,
    mirroring the backward finite difference v_bwd = (x_n - x_{n-1})/dt.
    This breaks the time-symmetry that causes ⟨v_sym a_sym⟩ = 0, recovering
    a non-zero and correctly-signed ⟨v a⟩ ∝ -1/γ.

    Only velocity columns are written; call compute_weak_derivatives separately
    for the acceleration (centred window).

    Parameters
    ----------
    vx_col, vy_col : str
        Output column names (default 'vx', 'vy' so they feed directly into the
        inference pipeline as the node velocity features).
    """
    if not inplace:
        data = data.copy()
    ...
    tbars          = np.linspace(-1.0, 1.0, tau + 1)
    delta          = 2.0 / tau
    pattern        = (np.array([1] + [4, 2] * (tau // 2 - 1) + [4, 1])
                      if tau >= 4 else np.array([1, 4, 1]))
    simpson_weights = (delta / 3.0) * pattern

    w_vals  = w(tbars)
    wp_vals = wp(tbars)

    Iw = np.sum(w_vals * simpson_weights)
    if np.isclose(Iw, 0.0):
        raise ZeroDivisionError("integral of w is ~0; choose a different test function.")
    ...
    for pid, dfp in data.sort_values([particle_col, frame_col]).groupby(particle_col, sort=False):
        ...
        for c in range(tau, n):
            win   = slice(c - tau, c + 1)
            x_win = x_arr[win]
            y_win = y_arr[win]
            if not (np.all(np.isfinite(x_win)) and np.all(np.isfinite(y_win))):
                continue
            data.loc[idx[c], vx_col] = scale_v * (-np.sum(x_win * wp_vals * simpson_weights) / Iw)
            data.loc[idx[c], vy_col] = scale_v * (-np.sum(y_win * wp_vals * simpson_weights) / Iw)
...
def calculate_derivatives(data, dt=0.1,weak_form = False,naive_form = False,tau = 16):
    ...
    if weak_form and naive_form:
        print("Calculating accelerations via weak and velocities via naive")
        compute_weak_derivatives(
            data=data,
            dt=dt,         # e.g. 0.1
            tau=tau,             # even integer; window spans tau*dt in time and uses tau+1 samples
